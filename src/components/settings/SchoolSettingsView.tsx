...
export const SchoolSettingsView: React.FC = () => {
  ...
  // Check if backup banner should be shown (first-run setup suggestion)
  useEffect(() => {
    const hasSeenBanner = localStorage.getItem('msps_backup_banner_dismissed');
    const isConnected = localStorage.getItem('msps_backup_connected');
    if (!hasSeenBanner && !isConnected) {
      const timer = setTimeout(() => setShowBackupBanner(true), 3000);
      return () => clearTimeout(timer);
    }
  }, []);

  const dismissBackupBanner = () => {
    localStorage.setItem('msps_backup_banner_dismissed', 'true');
  ...
  return (
    <div className="space-y-6 pb-16 text-[#1d1d1f]">
      ...
      {/* Tab switcher for Settings → Backup */}
      <div className="bg-white rounded-[18px] border border-[#e5e5ea] p-2 shadow-xs flex items-center gap-2 w-fit">
        <button
          onClick={() => setActiveTab('profile')}
          className={`px-4 py-2 rounded-full text-sm font-medium flex items-center space-x-2 transition-colors ${activeTab === 'profile' ? 'bg-[#0066cc] text-white' : 'text-[#86868b] hover:bg-[#f5f5f7] hover:text-[#1d1d1f]'}`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full text-sm font-medium flex items-center space-x-2 transition-colors ${activeTab === 'backup' ? 'bg-[#0066cc] text-white' : 'text-[#86868b] hover:bg-[#f5f5f7] hover:text-[#1d1d1f]'}`}
        >
          <Cloud className="h-4 w-4" />
          <span>Backup</span>
          <span className="bg-[#30d158] text-white text-[10px] px-1.5 py-0.5 rounded-full ml-1">New</span>
        </button>
      </div>

      {/* First-run backup suggestion banner */}
      {showBackupBanner && activeTab === 'profile' && (
        <div className="bg-gradient-to-r from-[#0066cc]/10 via-white to-[#30d158]/10 rounded-[18px] border border-[#0066cc]/20 p-5 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in fade-in">
          <div className="flex items-start space-x-3">
            <div className="h-10 w-10 rounded-full bg-[#0066cc]/10 flex items-center justify-center shrink-0">
              <ShieldCheck className="h-5 w-5 text-[#0066cc]" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-[#1d1d1f]">Protect your school data</h3>
              <p className="text-xs text-[#86868b] mt-1 max-w-md leading-relaxed">
                Connect Google Drive to automatically back up your school database with strong encryption. Your data stays safe even if your computer fails.
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2 shrink-0">
            ...
            <button onClick={dismissBackupBanner} className="p-1 text-[#86868b] hover:text-[#1d1d1f]">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
        ...
        <>
          ...
          <div className="bg-gradient-to-r from-[#0066cc]/5 via-white to-[#30d158]/5 rounded-[18px] border border-[#0066cc]/20 p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() => setActiveTab('backup')}
                className="apple-btn-secondary py-2 px-4 text-xs border-[#0066cc]/20 text-[#0066cc]"
              >
                <Cloud className="h-3.5 w-3.5 mr-1 shrink-0" />
